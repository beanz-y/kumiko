...
const KumikoCanvas = forwardRef(({ imageSrc, gridSize, baseThickness, sensitivity, activePattern, useOriginalColors }, ref) => {  const canvasRef = useRef(null);
  ...
  const renderGrid = (ctxOrBuilder, width, height, offscreenCtx) => {
    ...
    for (let y = -1; y < rows; y++) {
      for (let x = -1; x < cols; x++) {
        ...
        // Extract the exact RGB values of the pixel
        const pixelData = offscreenCtx.getImageData(sampleX, sampleY, 1, 1).data;
        const r = pixelData[0];
        const g = pixelData[1];
        const b = pixelData[2];
        
        // Calculate brightness for the line thickness
        ...
        // Apply Color: Either the photograph's RGB or the default wood color
  ...
  useImperativeHandle(ref, () => ({
    exportPNG: (targetHeight) => {
      ...
      // Calculate how much we need to scale up from our preview canvas
      const scaleFactor = targetHeight / height;
      const scaledWidth = width * scaleFactor;

      // Create a temporary high-res canvas in memory
      const tempCanvas = document.createElement('canvas');
      tempCanvas.width = scaledWidth;
      tempCanvas.height = targetHeight;
      const tempCtx = tempCanvas.getContext('2d');

      // Scale the drawing context so our existing math draws huge
      tempCtx.scale(scaleFactor, scaleFactor);

      // Render the background and the grid to the high-res canvas
      tempCtx.fillStyle = '#fdfbf7'; 
      tempCtx.fillRect(0, 0, width, height);
      tempCtx.strokeStyle = '#3e2723';
    ...
    exportSVG: (targetHeight) => {
      ...
      // Pass the scaleFactor so the SVG opens at the target resolution
